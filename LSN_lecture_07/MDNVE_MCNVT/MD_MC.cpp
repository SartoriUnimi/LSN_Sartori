...
int main()
{
  ...
  int nconf = 1;
  for(int i = 0; i < 5000; i++) //Equilibrate
      Move();
...
//Read initial configuration
  ...
  for (int i=0; i<npart; ++i)
  {
    ...
    x[i] = Pbc( x[i] * box );         //Scale configuration (reduced unts)
    y[i] = Pbc( y[i] * box );
    z[i] = Pbc( z[i] * box );
  }
  ReadConf.close();
...
// distance i-j in pbc
  ...
  //walker[ie] = walker[iv] + kin;  // Total energy
  walker[ip] = rho*walker[it] + 48. * f /((double)vol*3.) + ptail; // Pressure
...
void Averages(int iblk) //Print results for current block
{
   ...
   const int wd=12;
    ...
    Gdr.open("output_gdr_solid.dat", ios::app); 
